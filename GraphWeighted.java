 ...
 public void addEdge(NodeWeighted source, NodeWeighted destination, double weight) {
	...
	public String DijkstraShortestPath(NodeWeighted start, NodeWeighted end) {
	    ...
	    while (true) {
	        ...
	        if (currentNode == null) {
	            return "There are no flights available between " + start.name + " and " + end.name;
	        }
	        ...
	        if (currentNode == end) {
	            ...
	            return "The flight path with the fastest flight time between " + start.name + " and " + end.name + ":" +  "\n" + "The route is " + path + "\n Total flight time in hours: " + shortestPathMap.get(end);
	        }
	        currentNode.visit();
